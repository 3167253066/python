...
#导入数据集
filedata = np.array(pd. read_csv("E:\pythonProject4\letter-recognition.data"))

#调用numpy的 genfromtxt函数加载数据，两个参数‘delimiter’表示定界符，‘usecol’读取指定的列
...
from svm import SMO
#选用一组超参数进行试验，创建模型，选择惩罚系数，步长阈值，核函数，高斯核函数的gamma(绝对带宽大小)
clf = SMO(C=5, tol=0.01, kernel='rbf', gamma=0.05)
from sklearn.model_selection import train_test_split

#调用train_test_split函数，将数据集分为训练集和测试集，比例为7：3
...
for C in C_list:
    for gamma in gamma_list:
        clf = SMO(C=C, tol=0.01, kernel='raf', gamma=gamma)
       #训练预测并且计算精确度
        clf.train(X_train, y_train)
        y_pred = clf.predict(X_test)
       #保存精度以及相应参数
